  ...
  return (
    <nav className="sticky top-0 z-20 flex items-center justify-between gap-8 border-b border-line bg-ink px-6 py-4 sm:px-12">
      <Link to="/" className="flex items-center gap-3 shrink-0">
        <img src={PalsAnalytixLogo} alt="PalsAnalytix" className="block h-16 w-auto" />
        <div className="flex flex-col items-center">
          <img src={PalsAnalytixWordmark} alt="palsanalytix" className="block h-9 w-auto" />
...
const LandingPage = () => {
  ...
  // Used for both login and signup success -- closes the modal AND takes
  // the user straight into their dashboard, instead of leaving them on
  // the homepage to find/click the Dashboard button themselves. Reads
  // isAdmin from localStorage (set synchronously inside the loginUser
  // thunk) rather than from Redux state, matching the same reliable
  // pattern Navbar.jsx's handleNavigation already uses -- avoids a
  // possible stale-closure read of Redux state in the same tick the
  // login/signup dispatch resolves.
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
            {authMode === "login" ? (
              ...
              // BUG FIX: this was onSuccess={switchToLogin}, which just
              // flipped the modal back to the LOGIN form after a
              // successful signup+OTP verification -- instead of
              // recognizing the user as now genuinely authenticated
              // (which authSlice.js's verifyOTP.fulfilled now correctly
              // sets up). Now also navigates straight to the dashboard.
              <SignupModal onSuccess={handleAuthSuccess} onClose={closeModal} onLoginClick={switchToLogin} />
            )}
          </div>
        </div>
      )}
