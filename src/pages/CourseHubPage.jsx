...
const CourseHubPage = () => {
  ...
  const handleOptionClick = (key) => {
    if (key === "custom-assignments") {
      navigate(`/dashboard/course/${courseCode}/assignments/new`);
    }
  };
      ...
      <main className="flex-1 px-6 sm:px-12 py-16 pt-28 max-w-6xl mx-auto w-full">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {OPTIONS.map(({ key, title, description, icon: Icon }) => {
            const isLive = key === "custom-assignments";
            return (
              <div
                key={key}
              ...
              >
                ...
                {isLive ? (
                  <button
                    ...
                    className="w-full py-2.5 rounded-[3px] bg-brand-gradient text-charcoal font-semibold text-sm hover:opacity-90 transition"
                  >
                    Start
                  </button>
                ) : (
