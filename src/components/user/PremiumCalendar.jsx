...
export const StreakCalendar = ({
  ...
  const getActivityColor = () => {
    if (totalAttempts === 0) return "text-sand-500";
    if (totalAttempts < 10) return "text-accent-orange2";
    if (totalAttempts < 30) return "text-green-500";
    if (totalAttempts < 60) return "text-emerald-500";
    if (totalAttempts < 100) return "text-accent-amber";
    return "text-accent-orange";
  };

  return (
    <div className="bg-ink rounded-lg py-10 shadow-md overflow-hidden text-sand-300 p-4 font-sans">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-4">
          <h2 className="font-sora text-xl font-bold">
            <span className="text-paper">{totalAttempts}</span> submissions in
            the past one year
          </h2>
          {!isPremium && (
            <div className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-white/10 text-sand-300 border border-line-light">
        ...
        <div className="flex items-center space-x-6">
          <div className="text-sm">
            Total active days:{" "}
            <span className="text-paper font-medium">{totalAttempts}</span>
          </div>
          {/* <div className="text-sm">
      ...
      <div className="relative w-full py-4">
        {!isPremium && (
          <div className="absolute inset-0 bg-ink/80 backdrop-blur-sm flex flex-col items-center justify-center z-10 rounded-md">
            <Lock className="w-6 h-6 text-sand-400 mb-2" />
            <p className="text-sm font-medium text-sand-300">Premium Feature</p>
            <p className="text-s text-sand-500 mt-1 text-center px-4">
              <a href="#" onClick={(e) => { e.preventDefault(); navigate("/pricing"); }} className="text-accent-orange2 hover:text-accent-amber font-medium transition duration-200">
          ...
          {groupedByMonth.map((monthData, monthIndex) => {
            ...
            return (
              <div key={monthIndex} className="flex flex-col items-center px-1">
                        ...
                        return (
                          <div
                            ...
                            className={`w-3 h-3 rounded-sm my-0.5 mx-0.2 ${
                              isAttempted ? "bg-accent-orange2" : "bg-white/10"
                            }`}
                          >
                            <Tooltip
                ...
                <div className="mt-1 text-xs text-sand-500">{monthLabel}</div>
              </div>
            );
          })}
        </div>
