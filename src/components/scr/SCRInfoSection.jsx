  ...
  return (
    <section className="grid lg:grid-cols-2 gap-8 mb-12">
      <div className="bg-white border border-sand-200 p-6 rounded-lg">
        <h3 className="font-sora text-xl font-bold text-accent-orange2 mb-4">
          Roadmap to Earning the SCR® Certification
        </h3>
        <ul className="list-disc mt-4 ml-6 text-sand-900 space-y-3">
          {roadmapSteps.map((step, index) => (
            <li key={index} className="mb-3">
              <b className="text-charcoal">{step.title}</b> - {step.content}
            </li>
          ))}
        </ul>
      </div>
      <div className="bg-white border border-sand-200 p-6 rounded-lg">
        <h3 className="font-sora text-xl font-bold text-accent-amber mb-4">
          Frequently Asked Questions - SCR® Exam
        </h3>
        <ul className="list-disc mt-4 ml-6 text-sand-900 space-y-3">
          {faqs.map((faq, index) => (
            <li key={index} className="mb-3">
              <b className="text-charcoal">{faq.question}</b> - {faq.answer}
            </li>
          ))}
        </ul>
      </div>
