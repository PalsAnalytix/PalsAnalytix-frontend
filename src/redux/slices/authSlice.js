...
export const verifyOTP = createAsyncThunk(
  ...
  async ({ email, otp }, { rejectWithValue }) => {
    try {
      ...
      // BUG FIX: /api/verify-otp only returns a minimal user object (id,
      // username, email, phoneNumber) -- not the full profile (questions,
      // stats, subscription plan, etc.) that Dashboard.jsx actually reads
      // to render. That's why it showed blank until a manual refresh
      // triggered fetchUserProfile separately. Fetch the full profile
      // here too, mirroring exactly what loginUser already does below,
      // so both return the same complete shape.
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      // BUG FIX: this used to only flip otpVerified to true and throw away
      // the token/user the backend actually returns from /api/verify-otp.
      // That meant the app never considered you logged in after signup --
      // any protected route bounced straight back to the login screen,
      // even though verification itself had genuinely succeeded. Now this
      // mirrors what loginUser.fulfilled already does: store the token,
      // mark the user authenticated.
