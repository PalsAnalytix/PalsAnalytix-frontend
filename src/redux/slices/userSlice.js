...
export const fetchUserData = createAsyncThunk(
  ...
  async (userId, thunkAPI) => {
    try {
      ...
      if (
        userData.attemptedQuestions &&
        userData.attemptedQuestions.length > 0
      ) {
        console.log("Attempting to fetch question details");
        const questionDetails = await thunkAPI
          .dispatch(fetchBulkQuestionDetails(userData.attemptedQuestions))
          .unwrap();
        console.log(
          "Fetch successful, question details retrieved:",
          questionDetails
        );
